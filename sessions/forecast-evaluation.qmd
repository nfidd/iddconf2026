...
In this session we will use the `iddconf2026` package to load a data set of infection times and forecasts, the `dplyr` package for data wrangling, the `ggplot2` library for plotting and the `scoringutils` package for evaluating forecasts.
...
::: {.callout-tip .optional collapse="true"}
## Interactive: the CRPS

```{=html}
<iframe class="nfidd-embed" src="../interactive/forecast-evaluation.html?embed=crps" title="CRPS interactive" loading="lazy" style="width:100%;height:760px;border:1px solid #e5e7eb;border-radius:10px;background:#fff"></iframe>
...
Drag the observation and the forecast spread to watch the CRPS area — and its dispersion / over-prediction / under-prediction breakdown.
:::
...
::: {.callout-tip .optional collapse="true"}
## Interactive: the PIT histogram

```{=html}
<iframe class="nfidd-embed" src="../interactive/forecast-evaluation.html?embed=pit" title="PIT histogram interactive" loading="lazy" style="width:100%;height:1320px;border:1px solid #e5e7eb;border-radius:10px;background:#fff"></iframe>
<script>(function(){if(window.__nfiddEmbedInit)return;window.__nfiddEmbedInit=1;window.addEventListener('message',function(ev){var h=ev&&ev.data&&ev.data.nfiddEmbedHeight;if(!h)return;document.querySelectorAll('iframe.nfidd-embed').forEach(function(f){if(f.contentWindow===ev.source)f.style.height=(h+24)+'px';});});})();</script>
```

Build the histogram one forecast at a time — reading u = F(y) off each CDF — and see what U-shapes and domes are telling you.
:::
...
::: {.callout-tip .optional collapse="true"}
## Interactive: natural vs log-scale scoring

```{=html}
<iframe class="nfidd-embed" src="../interactive/forecast-evaluation.html?embed=logscale" title="natural vs log-scale scoring interactive" loading="lazy" style="width:100%;height:760px;border:1px solid #e5e7eb;border-radius:10px;background:#fff"></iframe>
<script>(function(){if(window.__nfiddEmbedInit)return;window.__nfiddEmbedInit=1;window.addEventListener('message',function(ev){var h=ev&&ev.data&&ev.data.nfiddEmbedHeight;if(!h)return;document.querySelectorAll('iframe.nfidd-embed').forEach(function(f){if(f.contentWindow===ev.source)f.style.height=(h+24)+'px';});});})();</script>
```

See why the natural-scale score is dominated by the peak, while the log scale weights growth, peak and decline more evenly.
:::
