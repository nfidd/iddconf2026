...
Different modelling approaches have different strengths and weaknesses, and we do not usually know in advance which one will produce the best forecast in any given situation.
...
In this session, we'll start with forecasts from three models and build ensembles of these models.
...
The aim of this session is to introduce the concept of ensembles of forecasts and to evaluate the performance of ensembles of three individual forecast models.
...
In this session we will use the `nfidd` package to load a data set of infection times and forecasts, the `dplyr` and `tidyr` packages for data wrangling, the `ggplot2` library for plotting and the `scoringutils` package for evaluating forecasts.
...
In this session we will use forecasts from three models.
They all share the same basic renewal with delays structure but use different models for the evolution of the effective reproduction number over time.
These were:
