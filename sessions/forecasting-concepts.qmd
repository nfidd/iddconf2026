...
A mechanistic model that estimates the reproduction number can be turned into a forecasting model by extending it into the future.
...
We forecast using a mechanistic model of the outbreak that we can simulate forward in time.

::: {.callout-note}
## The model we are forecasting with

The model is a discrete renewal process for infections.
The effective reproduction number $R_t$ evolves as a geometric random walk, so $\log R_t$ takes independent normal steps over time.
Each day's expected infections are given by the renewal equation, combining $R_t$ with past infections weighted by the generation time distribution.
Expected symptom onsets follow from convolving infections with a delay distribution from infection to onset.
The observed onsets are then modelled as Poisson draws around these expected onsets.
The random walk on $R_t$ lets the model track changes in transmission without assuming a fixed epidemic shape.
:::

This seems like a reasonable model for the data and so we might want to use it to forecast into the future.

We can do this by simulating the generative model forward in time.
To do this we use the same model but add a `generated quantities` block to create the forecasts.
...
We will use a simulated set of symptom onsets as this workshop's example data.
We will try to make a forecast on day 71, assuming we do not know what the data looks like after that day.
...
The only addition compared with fitting the model to data alone is a non-zero `h` in the data list, which sets the number of days to forecast.
