...
nfidd_stan_path <- function() {
  system.file("stan", package = "iddconf2026")
}
...
nfidd_stan_functions <- function(
    stan_path = iddconf2026::nfidd_stan_path()) {
...
nfidd_load_stan_functions <- function(
    functions = NULL, stan_path = iddconf2026::nfidd_stan_path(),
...
#' @examples
#' nfidd_stan_models()
nfidd_stan_models <- function(stan_path = iddconf2026::nfidd_stan_path()) {
  stan_files <- list.files(
    stan_path,
...
nfidd_cmdstan_model <- function(
    model_name = NULL,
    model_file = NULL,
    include_paths = getOption("nfidd.stan_path", iddconf2026::nfidd_stan_path()),
    ...) {
